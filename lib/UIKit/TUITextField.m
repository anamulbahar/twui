...
@interface TUITextFieldAutocorrectedPair : NSObject <NSCopying>
...
- (id)initWithFrame:(CGRect)frame
{
	if((self = [super initWithFrame:frame])) {
		...
		renderer = [[TUITextFieldEditor alloc] init];
...
{
	delegate = d;
	_textFieldFlags.delegateTextFieldDidChange = [delegate respondsToSelector:@selector(textFieldDidChange:)];
	_textFieldFlags.delegateDoCommandBySelector = [delegate respondsToSelector:@selector(textField:doCommandBySelector:)];
	_textFieldFlags.delegateWillBecomeFirstResponder = [delegate respondsToSelector:@selector(textFieldWillBecomeFirstResponder:)];
	_textFieldFlags.delegateDidBecomeFirstResponder = [delegate respondsToSelector:@selector(textFieldDidBecomeFirstResponder:)];
	_textFieldFlags.delegateWillResignFirstResponder = [delegate respondsToSelector:@selector(textFieldWillResignFirstResponder:)];
	_textFieldFlags.delegateDidResignFirstResponder = [delegate respondsToSelector:@selector(textFieldDidResignFirstResponder:)];
...
{	
	...
	rendererFrame.size.width = 20000.0f;
			...
			for(NSTextCheckingResult *result in results) {
				...
				if(result.resultType == NSTextCheckingTypeCorrection || result.resultType == NSTextCheckingTypeReplacement) {
					...
					if(NSMaxRange(result.range) <= backingString.length) {
						...
						TUITextFieldAutocorrectedPair *correctionPair = [[TUITextFieldAutocorrectedPair alloc] init];
...
- (NSMenu *)menuForEvent:(NSEvent *)event
{
	...
	TUITextFieldAutocorrectedPair *matchingAutocorrectPair = nil;
	if(selectedTextCheckingResult == nil) {
		for(TUITextFieldAutocorrectedPair *correctionPair in self.autocorrectedResults) {
...
- (BOOL)doCommandBySelector:(SEL)selector
{
	if(_textFieldFlags.delegateDoCommandBySelector) {
		BOOL consumed = [delegate textField:self doCommandBySelector:selector];
...
{
	if(self.rightButton != b) {
		[self.rightButton removeFromSuperview];
		self.rightButton = b;
		
		self.rightButton.layout = ^CGRect(TUIView *v) {
...
{
	if(_textFieldFlags.delegateWillBecomeFirstResponder) [delegate textFieldWillBecomeFirstResponder:self];
}

- (void)textRendererDidBecomeFirstResponder:(TUITextRenderer *)textRenderer
{
	if(_textFieldFlags.delegateDidBecomeFirstResponder) [delegate textFieldDidBecomeFirstResponder:self];
}

- (void)textRendererWillResignFirstResponder:(TUITextRenderer *)textRenderer
{
	if(_textFieldFlags.delegateWillResignFirstResponder) [delegate textFieldWillResignFirstResponder:self];
}

- (void)textRendererDidResignFirstResponder:(TUITextRenderer *)textRenderer
{
	if(_textFieldFlags.delegateDidResignFirstResponder) [delegate textFieldDidResignFirstResponder:self];
}

@end

@implementation TUITextFieldAutocorrectedPair

- (BOOL)isEqual:(id)object {
	if(![object isKindOfClass:[TUITextFieldAutocorrectedPair class]]) return NO;
	
	TUITextFieldAutocorrectedPair *otherPair = object;
...
- (id)copyWithZone:(NSZone *)zone {
	TUITextFieldAutocorrectedPair *copiedPair = [[[self class] alloc] init];
